...
const ApprovedContactRequests = () => {
  ...
  const approveContact = useMutation({
    mutationFn: async (CandidateID) => {
      const res = await axiosSecure.patch(`/contact-requests/approve/${CandidateID}`);
      return res.data;
    },
    onSuccess: () => {
  ...
  const handleApproveContact = (CandidateID) => {
    approveContact.mutate(CandidateID);
  };
  if (loading) {
    <p>Loading...</p>;
  }
  ...
  return (
    <>
      <Card>
        <Helmet>
          <title>Approved Contact Request | BB-Vote</title>
        </Helmet>
        <CardHeader>
